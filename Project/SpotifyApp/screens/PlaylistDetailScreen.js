...
const PlaylistDetailScreen = ({ route }) => {
  ...
  const moreAlbumsByArtist = []; // No "More by Artist" data for playlists
  const copyright = '';
  const songIdsRef = useRef([]); // Lưu trạng thái bài hát được like
  const playingTrackIdRef = useRef(null); // To store the ID of the currently playing track
  const trackRefs = useRef({}); // To store references to track items
  const convertToHoursAndMinutes = (totalDuration) => {
    const { minutes, seconds } = totalDuration;
  ...
  // Lấy danh sách bài hát được like từ API
  useEffect(() => {
  ...
  // Cập nhật màu sắc khi màn hình được mount
  useEffect(() => {
    // Lấy bài hát hiện tại từ MusicPlayerService
    const currentTrack = MusicPlayerService.currentSong;
    if (currentTrack) {
      const currentTrackId = currentTrack.track_id || currentTrack.trackId;

      // Đổi màu bài hát hiện tại
  ...
  useEffect(() => {
    const songChangedListener = MusicPlayerService.emitter.addListener(
      ...
      ({ currentSong }) => {
        ...
        // Reset the style of the previously playing track
        ...
        // Update the ref to the currently playing track
        playingTrackIdRef.current = currentTrackId;
      }
    );
...
const TrackItem = React.memo(({ item, handleTrackPress, songIds }) => {
  ...
  return (
    <TouchableOpacity style={styles.trackItem} onPress={() => handleTrackPress(item)}>
      ...
      <View style={styles.trackInfo}>
        <Text
          ref={(ref) => (trackRefs.current[item.track_id] = ref)}
          style={styles.trackTitle}
          numberOfLines={1}
        >
          {item.track_name}
